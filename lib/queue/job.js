...
exports.get = function(id, fn){
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    if (err) return fn(err);
	if (!hash) return fn(new Error('Job expired'));
...
Job.prototype.state = function(state){
  ...
  client.zadd('q:jobs', this._priority, this.id, noop);
  client.zadd('q:jobs:' + state, this._priority, this.id, noop);
  client.zadd('q:jobs:' + this.type + ':' + state, this._priority, this.id, noop);
  return this;
};
...
Job.prototype.update = function(fn){
  ...
  this.client.expire('q:job:' + this.id, 180, function(err) {
	  if (err) console.log('Expire setting error:', err);
  });
